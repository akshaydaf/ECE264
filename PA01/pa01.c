...
void test_my_strtol(char *str)
{
    ...
    for (base = 0; base < 38; base++) {
       ...
       if (my_endptr != NULL) {
          printf("My Base: %d, %ld:%p %s %d\n",base, test, my_endptr, my_endptr, my_errno);
       } else {
          printf("My Base: %d, %ld:NULL NULL %d\n",base, test, my_errno);
       }
       ...
       // set errno to zse: 0, 10:0x400e6c  ero
       ...
       if (c_endptr != NULL) {
          printf("C  Base: %d, %ld:%p %s %d\n",base, test, c_endptr, c_endptr, c_errno);
       } else {
          printf("C  Base: %d, %ld:NULL NULL %d\n",base, test, c_errno);
       }
    }
